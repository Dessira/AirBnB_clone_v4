$(document).ready(function () {
    ...
    new_sec.append(new_article);
     ...
     //target a button to check when it was clicked
     $('button').on('click', function(){
        // get all the boxes that have been checked
	//initialize variable tohold it
       let checked_val = [];
      ...
      url: "http://0.0.0.0:5001/api/v1/places_search/",
      type: "POST",
      data: JSON.stringify({amenities: checked_val}),
      contentType: "application/json",
      success: function(data){
        for (let place = 0; place < data.length; place++){
    console.log(data[place])
